...
public class ProductService {
    ...
    @Transactional
    public ApiResponse<?> createProduct(CreateProductRequest req, List<MultipartFile> images) {
        try {
    ...
    public PagedModel<EntityModel<ProductDto>> getAllProducts(int page, int size) {

        return pagedResourcesAssembler.toModel(productRepository
                .findAllByIsActiveTrueOrderByCreatedAtDesc(PageRequest.of(page,size))
                .map(ProductDto::toDto));
    }

    public PagedModel<EntityModel<ProductDto>> getAllAdminProducts(int page, int size) {
    ...
    public List<ProductDto> getAllNewSeasonProduct(int page, int size) {
        return productRepository.findByNewSeasonTrueAndIsActiveTrue(PageRequest.of(page,size))
                .stream().map(ProductDto::toDto).collect(Collectors.toList());
    }

    public List<ProductDto> getAllPopulateProduct(int page, int size) {
        return productRepository.findByPopulateTrueAndIsActiveTrue(PageRequest.of(page,size))
    ...
    @Transactional
    public ApiResponse<?> updateProduct(String dataJson, List<MultipartFile> images) {
        try {
    ...
    public PagedModel<EntityModel<ProductDto>> filterProducts(String subCategory
        ...
        return pagedResourcesAssembler.toModel(
                productRepository.findProductsByFilters(subCategory,PageRequest.of(page, pageSize))
                        .map(ProductDto::toDto)
        );
    }
